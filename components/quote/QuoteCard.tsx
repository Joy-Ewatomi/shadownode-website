...
type QuoteCardProps = {
  request: {
    ...
    status: string
    created_at: string
    ai_price_estimate?: string | number | null
...
export default function QuoteCard({ request, onAccept, onReview, onDecline }: QuoteCardProps) {
  ...
  return (
    <article className="px-5 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <RequestStatusBadge status={request.status || "pending_review"} />
      </div>
      <p className="mt-3 text-sm text-white/60">{request.description}</p>
      <div className="mt-4">
        <QuoteSummary
          aiEstimate={request.ai_price_estimate}
          aiReasoning={request.ai_reasoning}
          quoteAmount={quoteAmount}
          currency={request.approved_quote_currency || "NGN"}
          notes={request.approved_quote_notes || request.quote_notes}
          estimatedCompletion={request.approved_estimated_completion}
        />
      </div>
      <QuoteDecisionButtons canDecide={Boolean(quoteAmount && ["quote_sent", "revised_quote_sent"].includes(request.status))} onAccept={onAccept} onReview={onReview} onDecline={onDecline} />
    </article>
  )
}
