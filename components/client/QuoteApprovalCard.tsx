...
export default function QuoteApprovalCard({ request }: Props) {
...
if (decision === "accept") {
  setMessage(
    "Quote accepted. Redirecting you to secure payment...",
  )
...
setDecisionMade(true)
  ...
  async function submitBargain() {
    ...
    if (!Number.isFinite(budget) || budget <= 0) {
      setMessageType("error")
      setMessage("Please enter a valid proposed budget.")
      return
    }
    ...
    try {
      ...
      if (!res.ok) {
        setMessageType("error")
        setMessage(data.error || "Unable to submit negotiation.")
        return
      }
...
setDecisionMade(true)
      ...
      console.error("QUOTE NEGOTIATION ERROR:", error)
    ...
    return (
      <div className="mt-6 rounded-md border border-[#143b28] bg-black/20 p-4">
        <p className="text-sm text-white/50">
          This quote is no longer awaiting a client decision.
        </p>
      </div>
    )
  }
    ...
    <div className="mt-6 rounded-md border border-[#143b28] bg-black/20 p-5">
      <div className="mb-4">
        ...
        <p className="mt-2 text-sm leading-6 text-white/55">
          You may accept the quote, propose a different budget with
          a reason, or decline the quote.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button
          type="button"
          disabled={loading}
          onClick={() => setShowBargain((value) => !value)}
        ...
        >
          ...
          {showBargain ? "Cancel Proposal" : "Propose Different Budget"}
        </button>
        ...
        <div className="mt-5 rounded-md border border-[#143b28] bg-black/30 p-5">
          <div className="mb-4">
            ...
            <p className="mt-1 text-xs leading-5 text-white/45">
              Explain your proposed budget and the reason for your
              request. Your proposal will be reviewed before a final
              quote is issued.
            </p>
          </div>
            ...
            <div>
              ...
              <div className="flex">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setRequestedBudget(event.target.value)
                  }
