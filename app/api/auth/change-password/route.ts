...
import { auditLog, getCurrentUser, hashPassword, validatePassword, verifyPassword } from "@/lib/auth"
...
export async function POST(request: NextRequest) {
  ...
  if (!validatePassword(newPassword)) return NextResponse.json({ error: "Password must be 12+ characters and include uppercase, lowercase, number, and symbol." }, { status: 400 })
