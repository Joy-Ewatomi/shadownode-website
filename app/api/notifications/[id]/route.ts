...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await query(
      ...
      WHERE n.id = $1 AND (n.user_id = $2 OR $3 = 'super_administrator')
      LIMIT 1
      `,
    ...
    return NextResponse.json({ ...row, metadata: parsedMetadata })
  } catch (error) {
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await query(
      ...
      WHERE id = $1 AND (user_id = $2 OR $3 = 'super_administrator')
      RETURNING id
      `,
      [id, user.id, user.role],
    )
